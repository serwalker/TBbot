...
import pytz
import requests
from pyrogram.errors import PeerIdInvalid, YouBlockedUser
from pyrogram.types import Message


class RawFunctions(object):
    async def aexec(self, message: Message, code: str):
        """
        params:
    ...
    async def error(self, message: Message, e, edit_error: bool = True):
        """
        params:
    ...
    async def sleep(self, message: Message, sec: int = 0, delmsg=False):
        """
        params:
                1. message (update) :: incoming update
    ...
    async def delete_message(self, message: Message, sec: int = 0):
        """
        params:
                1. message (update) :: incoming update
    ...
    async def data(self, modules: str):
        """
        params:
        ...
        try:
            ...
            for x, y in zip(
                ...
                module_data.append(f"CMD: `{self.PREFIX}{x}`\nINFO: `{y}`\n\n")
            return module_data
        except Exception as e:
            self.log.error(e)
            return None
    ...
    async def check_private(self, message: Message):
        """
        params:
                1. message (update) :: incoming update
    ...
    def textlen(self, message: Message):
        """
        params:
                1. message (update) :: incoming update
            ...
            if send:
                if caption:
                    await self.send_document(
                        ...
                        caption=caption
                        if caption
                        else f"**Uploaded By:** {self.UserMention()}",
                    )
    ...
    def rem_dual(self, list1: list, list2: list):
        """
        params:
    ...
    async def kick_user(
        self, chat_id: Union[str, int], user_id: Union[str, int], ban_time: int = 30
    ):
        """
        params:
        ...
        try:
            return await self.ban_chat_member(
                chat_id, user_id, int(time.time()) + ban_time
            )
        except Exception as e:
    ...
    async def get_last_msg(self, m: Message, chat_id, reverse: bool = False):
        """
        params:
                1. message (update) :: incoming update
    ...
    async def toggle_inline(self, message: Message):
        """
        params:
                1. message (update) :: incoming update
        ...
        try:
            botname = "BotFather"
            await self.send_edit(
                message, "Processing command . . .", text_type=["mono"]
            )
            ...
            await self.send_edit(
                message, "Pressing Bot Settings . . . ", text_type=["mono"]
            )
            ...
            await self.send_edit(
                message,
                "checking whether inline mode is On or Off . . . ",
                text_type=["mono"],
            )
            ...
            if "Turn on" in str(data[0]):
                await self.send_edit(
                    message, "Turning Inline mode on . . . ", text_type=["mono"]
                )
                await data[0].click("Turn on")
                await self.send_edit(
                    message,
                    "Inline mode is now turned On.",
                    text_type=["mono"],
                    delme=4,
                )
            # Turn inline mode off
                ...
                await self.send_edit(
                    message, "Turning Inline mode Off . . .", text_type=["mono"]
                )
                await data[0].click("Turn inline mode off")
                await self.send_edit(
                    message,
                    "Inline mode is now turned Off.",
                    text_type=["mono"],
                    delme=4,
                )
    ...
    async def add_users(
        self, user_id: Union[int, str, List[int], List[str]], chat_id: Union[int, str]
    ):
        """
        params:
    ...
    async def user_exists(self, user_id: Union[int, str], chat_id: Union[int, str]):
        """
        params:
            ...
            if self.bot:
                ...
                try:
                    if await self.user_exists(self.bot.id, self.LOG_CHAT) is False:
                        ...
                        self.log.info(
                            f"COMPLETED: Bot is already present in log chat . . .\n"
                        )
                except PeerIdInvalid:
                    self.log.info(
                        "Peer id is invalid, Manually add bot to your log chat . . .\n"
                    )

            else:
                self.log.warning(
                    "Bot client is not available, please check (TOKEN, API_ID, API_HASH)"
                )
        except Exception as err:
    ...
    def import_module(self, path: str, exclude: list = [], display_module: bool = True):
        """
        params:
        ...
        ex:
                app.import_module("./asterix/modules/", exclude=["admin"])
        """
