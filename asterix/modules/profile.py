...
async def get_full_user_info(_, m: Message):
    ...
    try:
        ...
        if p_id:
            await app.send_cached_media(m.chat.id, file_id=p_id, caption=duo)
            await msg.delete()
        elif p_id is False:
...
async def tgscan_handler(_, m: Message):
    if m.reply_to_message:
        ...
        await app.forward_messages(
            "@tgscanrobot", m.chat.id, m.reply_to_message.message_id
        )
        time.sleep(0.5)
        msg = await app.get_history("@tgscanrobot", limit=1)
        if msg:
        ...
        await app.send_edit(
            m, "reply to someone's message . . .", delme=2, text_type=["mono"]
        )
...
async def userhistory_handler(_, m: Message):
    ...
    if not reply:
        ...
        await app.forward_messages("@SangMataInfo_bot", m.chat.id, reply.message_id)
        is_no_record = False
        for x in range(8):
            time.sleep(1)
            ...
            if msg[0].text == "No records found":
                ...
                break
            if (
                msg[0].from_user.id == 461843263
                and msg[1].from_user.id == 461843263
                and msg[2].from_user.id == 461843263
            ):
                await app.read_history("@SangMataInfo_bot")
                break
            else:
...
async def setprofile_handler(_, m: Message):
    ...
    if app.long(m) < 3:
        return await app.send_edit(
            m,
            "Please use text and suffix after command suffix: `fname`, `lname`, `bio`",
        )
    # set -> fname, lname & bio
    if app.long(m) > 2:
        ...
        if custom[1] in ["fname", "lname", "bio"]:
            await setprofile(m, custom[1], f"{text[2:]}")
        elif custom[1] == "uname":
...
async def setprofile(m: Message, args, kwargs):
    ...
    if args == "fname":
        try:
            await app.update_profile(first_name=f"{kwargs}")
            await app.send_edit(m, f"✅ Updated first name to [ {kwargs} ]")
        except Exception as e:
            await app.error(m, e)
    # set last name
    elif args == "lname":
        try:
            await app.update_profile(last_name=f"{kwargs}")
            await app.send_edit(m, f"✅ Updated last name to [ {kwargs} ]")
        except Exception as e:
            await app.error(m, e)
    # set bio
    elif args == "bio":
        try:
            await app.update_profile(bio=f"{kwargs}")
            await app.send_edit(m, f"✅ Updated bio to [ {kwargs}]")
        except Exception as e:
            await app.error(m, e)
    else:
...
=======
        m,
        "[Here Is Asterix Userbot Repo](https://github.com/TigerNetwork/Tiger-USERBOT)",
>>>>>>> ea066f0d17c2b9a16baf406fdaf634eef615f8bc
    )
